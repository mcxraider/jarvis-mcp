...
export function validateTextLength(text: string, maxLength: number): void {
  if (text.length > maxLength) {
    throw new Error(
      `Text length (${text.length} characters) exceeds maximum allowed length (${maxLength} characters)`,
    );
  }
}
 ...
 */
export function validateTextContent(text: string, minLength: number = 1): void {
