...
export class TelegramProgressService {
  ...
  async updateProgress(job: JobRecord, eventType: string, message?: string): Promise<void> {
    ...
    try {
      await this.responseService.updateMessage(job.chatId, Number(job.ackMessageId), progressText);
      await this.jobStateService.updateProgressMessage(job.id, job.ackMessageId);
    } catch (error) {
